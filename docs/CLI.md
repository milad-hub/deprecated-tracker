...
| `--whole-files` | With `--files`, scan the whole file and ratchet it per-file instead of reporting only changed lines |
...
FAIL — 1 deprecated usage(s) on lines this commit changed
```
