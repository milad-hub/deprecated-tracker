...
describe('Scanner', () => {
    ...
    beforeEach(() => {
        ...
        scanner = new scanner_1.Scanner(ignoreManager);
    });
    afterEach(() => {
    ...
    describe('scanProject', () => {
        it('should throw error if tsconfig.json not found', async () => {
            await expect(scanner.scanProject(workspaceFolder)).rejects.toThrow('tsconfig.json not found in workspace root');
        });
        it('should scan project and find deprecated items', async () => {
            ...
            fs.writeFileSync(testFile, `export class TestClass {
          /**
...
`);
            ...
            fs.writeFileSync(testFile, `import { Observable } from 'rxjs';
        ...
        export class TestRxJSSubscribe {
          ...
          private deprecatedMethod(): void {
            ...
            const subscribeResults = results.filter(r => r.name === 'subscribe' &&
                r.filePath === testFile);
            expect(subscribeResults.length).toBe(0);
            // Should find at least one explicitly deprecated method (usage or declaration)
            const deprecatedResults = results.filter(r => r.name === 'deprecatedMethod' &&
                r.filePath === testFile);
